...
for i=0:length(EbN0)
   ...
   for m=1:N_iter
      ...
      X = round(rand(1,Nused*Nframe*Nbps));           % 原始数据
      % Xmod= qammod(X,M,0,'gray')/norms(Nbps);
      Xmod = qammod(X', M, 'InputType', 'bit', 'UnitAveragePower', true)';
      if NgType~=2
      ...
      X_r = qamdemod(Xmod_r', M,'UnitAveragePower', true,'OutputType','bit')';
